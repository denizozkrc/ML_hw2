...
def myKMedoidsFunction(dataset):
    avg = np.zeros(11)
    avg[0] = avg[1] = np.inf
    sil_avg = np.zeros(11)
    ...
    for k_val in range(2, 11): 
        ...
        for repeat_and_average in range(0, 10):
            ...
            for repeat_and_lowest in range(0, 10):
                kmedoids = KMedoids(n_clusters=k_val, random_state=0)
                prediction = kmedoids.fit_predict(dataset)
                sil_score = silhouette_score(dataset, prediction)
                if kmedoids.inertia_ < smallest:
                    smallest = kmedoids.inertia_
...
myKMedoidsFunction(dataset1)
myKMedoidsFunction(dataset2)
